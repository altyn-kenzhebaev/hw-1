...
```
В секции provisioners указываем каким образом и какие действия необходимо произвести для настройки виртуальой машины. Именно в этой секции мы и обновим ядро системы, чтобы можно было получить образ с 6й версией ядра. Настройка системы выполняется несколькими скриптами, заданными в секции scripts.
```
          "scripts": [
...
```
Для появившегося Vagrantfile произведем замену значения box_name на имя импортированного образа. Соотвествующая строка примет вид:
```
:box_name => "centos8-kernel6",
```
и отключим функцию VirtualBox Shared Folders, добавляя следующую строку в конфигурацию:
```
